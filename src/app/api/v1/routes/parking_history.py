...
async def list_scans(
    ...
    items = await service.get_filtered(
        skip, limit, location_id, user_location_ids, start, end
    )
    total = await service.count_filtered(
        location_id, user_location_ids, start, end
    )
...
async def export_scans_csv(
    ...
    items = await service.get_filtered(
        0, 50000, location_id, user_location_ids, start, end
    )
...
async def export_scans_pdf(
    ...
    items = await service.get_filtered(
        0, 5000, location_id, user_location_ids, start, end
    )
